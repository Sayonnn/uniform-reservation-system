...
$size1 = isset($_POST["sizes"]) ? $_POST["sizes"] : '';
$price1 = $_POST["price"];
...
if (move_uploaded_file($tempname, $folder)) {
    $sql = "UPDATE tbproductinfo SET `itemid` = '{$id1}', `itemname` = '{$item1}',`item_img` = '{$filename}',`sizes` = '{$size1}',`price` = '{$price1}' WHERE `itemid` = '{$id1}'";
    $query = mysqli_query($conn,$sql);
     //   echo "New record created successfully";
     if($query){
      header("Location: viewitems.php?message=Updated&cardid={$id1}");
     }else {
...
<form  method="post" enctype="multipart/form-data">
    ...
    <input type="number" name="price" min="0" step="0.01" value="<?=$price ?>"><br>
